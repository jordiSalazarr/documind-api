...
func NewBaseEvent(eventType EventType, workspaceID ID) BaseEvent {
	return BaseEvent{
		ID:          NewID().String(),
		Type:        eventType,
...
func NewItemCreated(workspaceID, itemID, versionID, projectID ID) *ItemCreated {
	return &ItemCreated{
...
func NewVersionCreated(workspaceID, itemID, versionID ID, version int) *VersionCreated {
...
func NewPreviousVersionDeprecated(workspaceID, itemID ID, deprecatedVersion int) *PreviousVersionDeprecated {
...
func NewItemPublished(workspaceID, itemID, versionID ID) *ItemPublished {
...
func NewSourceLinkedToItem(workspaceID, itemID, sourceID ID) *SourceLinkedToItem {
...
func NewRelationCreated(workspaceID, relationID, fromItemID, toItemID, relationTypeID ID) *RelationCreated {
...
func NewEmbeddingRequested(workspaceID, itemID, versionID ID, content string) *EmbeddingRequested {
...
func NewEmbeddingGenerated(workspaceID, versionID ID, embedding []float32) *EmbeddingGenerated {
...
func NewChunksCreated(workspaceID, itemID, versionID ID, chunkIDs []ID) *ChunksCreated {
...
func NewChunkEmbeddingsRequested(workspaceID, itemID, versionID ID, chunkIDs []ID) *ChunkEmbeddingsRequested {
	strIDs := make([]string, len(chunkIDs))
	for i, id := range chunkIDs {
		strIDs[i] = id.String()
	}
...
func NewChunkEmbeddingsGenerated(workspaceID, versionID ID, chunkCount, tokensUsed, successCount, failureCount int) *ChunkEmbeddingsGenerated {
