...
type Repository interface {
	...
	CreateItemVersionWithHash(ctx context.Context, version *itemdomain.ItemVersion, contentHash string) error
	ProjectExists(ctx context.Context, id string) (bool, error)
	ItemTypeExists(ctx context.Context, id string) (bool, error)
	FindByContentHash(ctx context.Context, workspaceID, contentHash string) (itemID, versionID string, found bool, err error)
}
...
func (r *postgresRepo) CreateItemVersionWithHash(ctx context.Context, version *itemdomain.ItemVersion, contentHash string) error {
	customFieldsJSON, err := json.Marshal(version.CustomFields)
	if err != nil {
		return fmt.Errorf("failed to marshal custom fields: %w", err)
	}
	...
	_, err = r.db.ExecContext(ctx, query,
		version.ID, version.ItemID, version.WorkspaceID, version.Version,
		version.Title, version.Summary, version.BodyMd, customFieldsJSON, pq.Array(version.Tags), version.Status,
		version.CreatedAt, version.CreatedBy, contentHash,
	)
...
func (r *postgresRepo) FindByContentHash(ctx context.Context, workspaceID, contentHash string) (string, string, bool, error) {
	query := `
		SELECT iv.item_id, iv.id
		FROM item_versions iv
		JOIN items i ON i.id = iv.item_id
		WHERE iv.workspace_id = $1
		  AND iv.content_hash = $2
	...
	err := r.db.QueryRowContext(ctx, query, workspaceID, contentHash).Scan(&itemID, &versionID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return "", "", false, nil
		}
		return "", "", false, fmt.Errorf("failed to find by content hash: %w", err)
	}

	return itemID, versionID, true, nil
}
